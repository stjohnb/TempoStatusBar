...
- **Runners:** GitHub-hosted `macos-15` for build/test/quality jobs; `[self-hosted, linux]` for security scans, documentation checks, PR cleanup, and failure notification. GitHub-hosted macOS CI minutes are limited and billed, so batch changes and minimise the number of PRs.
...
|---|---|---|
| `main-build` | `macos-15` | Import signing cert → Release build + unit tests + Archive + verify signature + DMG creation → clean up keychain |
...
| Job | Runner | Description |
|---|---|---|
| `release-build` | `macos-15` | Import signing cert → Release build + Archive (both with `MARKETING_VERSION=<tag>` override) + verify signature + DMG (named `TempoStatusBarApp-<version>.dmg`) → attach raw DMG to the GitHub Release page → clean up keychain |
...
- **Split runners — GitHub-hosted macOS, self-hosted Linux** — build/test/quality jobs run on GitHub-hosted `macos-15` (the image ships the pinned Xcode 26.x and pre-trusts Apple's intermediate CAs for codesigning). The Linux-only utility jobs (Trivy scans, docs checks, PR cleanup, failure notification) run on `[self-hosted, linux]` and declare an explicit OS label rather than bare `self-hosted` so they are never scheduled onto a macOS runner. GitHub-hosted macOS minutes are limited and billed, so changes are batched and the number of PRs is kept low.
