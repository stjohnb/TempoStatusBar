...
All workflows use `xcode-version: '26.x'`. PR and main workflows use `cancel-in-progress: true`; the release workflow uses `cancel-in-progress: false`. Build/test/quality jobs run on GitHub-hosted `macos-15`; utility jobs (security scans, docs checks, PR cleanup, failure notification) run on `[self-hosted, linux]`. GitHub-hosted macOS CI minutes are limited and billed, so changes are batched and the number of PRs is kept low.
